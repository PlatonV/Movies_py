	...
	def executeCommand(self, command):
		if command.getType() == "add":
			...
			if len(command.getParams()) == 4:
				command.setPrevObj(self.update_movie(command.getParams()[0], command.getParams()[1], command.getParams()[2], command.getParams()[3]))
				command.setObj(self.search_movie(int(command.getParams()[0])))
			else:
				raise ValueError("Invalid parameters!")
